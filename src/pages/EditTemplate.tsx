  ...
  return (
    <div className="max-w-[1600px] mx-auto pb-32 px-4 md:px-8">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 md:gap-12 items-start">
        ...
        <div className="lg:col-span-5 xl:col-span-4 space-y-8 lg:sticky lg:top-36">
          ...
          <section className="bg-white/60 backdrop-blur-sm p-8 md:p-10 rounded-[40px] border border-slate-50 shadow-sm space-y-8">
            ...
            >
                ...
                <div className={`w-6 h-6 rounded-full border-2 flex items-center justify-center transition-all ${includeVehicleInfo ? 'border-indigo-600 bg-indigo-600' : 'border-slate-200 bg-transparent'}`}>
                    {includeVehicleInfo && <span className="material-symbols-outlined text-white text-[14px] font-black">check</span>}
                </div>
            </button>
          </section>
          ...
          <div className="hidden lg:block">
            <button 
                onClick={handleSave}
                className="w-full bg-[#0f172a] text-white py-10 rounded-[44px] font-black text-lg tracking-[0.3em] uppercase flex items-center justify-center gap-5 shadow-2xl hover:bg-slate-800 hover:-translate-y-2 active:translate-y-0 transition-all group"
            >
        ...
        <div className="lg:col-span-7 xl:col-span-8 space-y-8">
          ...
          <div className="space-y-8 md:space-y-10 min-h-[500px]">
            {fields.map((field, idx) => {
              ...
              return (
                ...
                >
                  ...
                  {/* Options Selection Container - REFATORADO PARA MOCKUP V2.5 + CATÁLOGO INTERATIVO */}
                  {['SELECT_SIMPLE', 'SELECT_PRICE', 'MULTI_SELECT'].includes(field.type) && (
                    <div className="space-y-10 bg-slate-50/30 p-8 md:p-14 rounded-[44px] md:rounded-[56px] border border-slate-100">
                      ...
                      {/* Box de Resumo Visual (CATÁLOGO) no Topo */}
                      ...
                      {/* Botões de Categoria (Auto-preenchimento) */}
                      ...
                      {/* Lista de Edição Detalhada - Onde o usuário define PREÇOS */}
            ...
            {fields.length === 0 && (
                <div className="bg-slate-50 rounded-[64px] border-4 border-dashed border-slate-100 h-[600px] flex flex-col items-center justify-center text-slate-200 gap-8 p-12 text-center">
                    <div className="w-28 h-28 md:w-40 md:h-40 bg-white rounded-full flex items-center justify-center shadow-inner">
                      <span className="material-symbols-outlined text-6xl md:text-8xl text-slate-100">playlist_add</span>
                    </div>
                    <div>
                      <p className="font-black text-sm md:text-lg tracking-[0.5em] uppercase text-slate-300">Arquitetura Vazia</p>
                      <p className="text-slate-200 font-bold text-xs md:text-sm mt-2 tracking-widest uppercase">Comece a adicionar componentes para construir seu checklist</p>
                    </div>
                </div>
            )}
          </div>
      ...
      <div className="lg:hidden pt-20">
        <button 
            onClick={handleSave}
            className="w-full bg-[#0f172a] text-white py-12 rounded-[52px] font-black text-xl tracking-[0.4em] uppercase flex items-center justify-center gap-6 shadow-2xl hover:bg-slate-800 transition-all active:scale-95"
        >
